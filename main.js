...
function initUploadZone() {
  ...
  function handleFileSelect(file) {
    ...
    if (!CONFIG.ALLOWED_EXTS.includes(ext)) {
      toast('Formato no soportado. Usá PDF, Word o TXT.', 'error');
      return;
    }

    if (sizeMB > CONFIG.MAX_FILE_MB) {
      toast(`El archivo supera los ${CONFIG.MAX_FILE_MB} MB.`, 'error');
      return;
    }
...
function initRealtimeValidation() {
  const rules = {
    nombre:   v => v.trim().length >= 2 ? null : 'Ingresá tu nombre completo.',
    email:    v => /^[^\s@]+@endeavor\.org$/i.test(v.trim()) ? null : 'Usá tu email @endeavor.org.',
    oficina:  v => v ? null : 'Seleccioná tu oficina.',
    audiencia:v => v ? null : 'Seleccioná la audiencia del video.',
    conceptos:v => v.trim().length >= 20 ? null : 'Describí los conceptos con al menos 20 caracteres.',
  };
...
function validateAll() {
  ...
  const checks = [
    { id: 'nombre',    errId: 'err-nombre',    fn: v => v.trim().length >= 2,           msg: 'Ingresá tu nombre completo.' },
    { id: 'email',     errId: 'err-email',     fn: v => /^[^\s@]+@endeavor\.org$/i.test(v.trim()), msg: 'Usá tu email @endeavor.org.' },
    { id: 'oficina',   errId: 'err-oficina',   fn: v => !!v,                            msg: 'Seleccioná tu oficina.' },
    { id: 'audiencia', errId: 'err-audiencia', fn: v => !!v,                            msg: 'Seleccioná la audiencia.' },
    { id: 'conceptos', errId: 'err-conceptos', fn: v => v.trim().length >= 20,          msg: 'Describí los conceptos (mínimo 20 caracteres).' },
  ];
  ...
  if (!selectedFile && !url) {
    showError('err-doc', null, 'Adjuntá un documento o pegá un link.');
    valid = false;
  } else {
    clearError('err-doc');
  }
...
async function submitToN8N() {
  ...
  try {
    ...
    if (err.name === 'TypeError' && err.message.includes('fetch')) {
      toast('No se pudo conectar con el servidor. Verificá tu conexión.', 'error');
    } else if (err.message.startsWith('Error 4')) {
      toast('Datos inválidos. Revisá el formulario y reintentá.', 'error');
    } else {
      toast('Ocurrió un error inesperado. Reintentá en unos segundos.', 'error');
    }
  } finally {
    isSubmitting = false;
...
function setLoadingState(loading) {
  ...
  label.textContent = loading ? 'Enviando…' : 'Enviar solicitud';
...
function showSuccess(solicitudId) {
  ...
  idEl.textContent = 'ID de solicitud: ' + solicitudId;
  card.hidden = false;
